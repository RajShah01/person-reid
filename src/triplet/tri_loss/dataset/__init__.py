...
def create_dataset(
  ...
  if name == 'market1501':
    im_dir = ospeu('/content/reid_data/market/images')
    partition_file = ospeu('/content/reid_data/market/partitions.pkl')

  elif name == 'cuhk03':
    im_type = ['detected', 'labeled'][0]
    im_dir = ospeu('/content/reid_data/cuhk/images')
    partition_file = ospeu('/content/reid_data/cuhk/partitions.pkl')

  elif name == 'duke':
    im_dir = ospeu('/content/reid_data/duke/images')
    partition_file = ospeu('/content/reid_data/duke/partitions.pkl')

  elif name == 'combined':
    assert part in ['trainval'], \
      "Only trainval part of the combined dataset is available now."
    im_dir = ospeu('/content/reid_data/trainval_images')
    partition_file = ospeu('/content/reid_data/partitions.pkl')
