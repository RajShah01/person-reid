...
def cmc(
  ...
  num_valid_queries = 0
  # try:
  #   print(type(matches), matches[i, valid], matches)
  # except:
  #   print("lafda",matches)
  c=0
  for i in range(m):
...
def mean_ap(
  ...
  is_valid_query = np.zeros(m)
  for i in range(m):
    #print(gallery_ids.shape, gallery_cams.shape,indices.shape, query_ids.shape, query_cams.shape, gallery_ids[indices[i]], query_ids[i],indices[i])
    #print("X",gallery_ids[indices[i][0]],query_ids[i],gallery_cams[indices[i][0]],query_cams[i])
    # Filter out the same id and same camera
    try:
      valid = ((gallery_ids[indices[i]] != query_ids[i]) |
             (gallery_cams[indices[i]] != query_cams[i]))
